...
(async () => {
  ...
  const url = "https://www.test.com.test/test/test/";
  const urlImage = "https://www.test.com.test";
  ...
  for (const productNode of productNodes) {
    const checkTitle = await page.evaluate(
      (element) =>
        element.querySelector('span[itemprop="name"]')?.textContent ||
        "No Title",
      productNode
    );
    ...
    if (checkTitle != "No Title") {
      productDetailsArray.push(productDetails);
    }
  }
  console.log(productDetailsArray);

  const outputFileName = "test_data.json";
  fs.writeFileSync(
    outputFileName,
  ...
  for (const productNode of productDetailsArray) {
    ...
    const imageFullUrl = urlImage + productNode.sourceFilename;
